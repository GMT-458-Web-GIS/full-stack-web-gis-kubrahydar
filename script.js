// script.js - Geo World Guess Game (with Auth + DB score saving)
// All UI text is English only.

// ================== API / AUTH ==================
const API_BASE = "http://localhost:4000";
const LS_TOKEN = "geoguess_token";
const LS_USER = "geoguess_user";

let authToken = null; // JWT
let authUser = null;  // {id,email,role}
...
async function apiJson(path, { method="GET", body=null, headers={} } = {}){
  ...
  if (res.status === 401){
    setAuth(null, null);
    showIntroAuth("Session expired. Please login again.");
...
function showIntroAuth(message){
  const intro = $("intro");
  if (intro) intro.classList.remove("hidden");
  $("introAuth")?.classList.remove("hidden");
  $("introConfig")?.classList.add("hidden");

  if (message) setAuthMessage(message, false);
}

function showIntroConfig(){
  $("introAuth")?.classList.remove("hidden");
  $("introConfig")?.classList.remove("hidden");
  $("introConfig")?.classList.remove("hidden");
  $("introConfig")?.classList.remove("hidden");
  $("introConfig")?.classList.remove("hidden");
  // Actually: hide auth step box visually? we keep it but config appears.
  // If you want auth box hidden, uncomment:
  // $("introAuth")?.classList.add("hidden");
  $("introConfig")?.classList.remove("hidden");
}
...
function setAuthMessage(msg, ok){
  ...
  if (!el) return;
  el.textContent = msg;
  el.style.borderColor = ok ? "rgba(34,197,94,0.45)" : "rgba(239,68,68,0.35)";
...
function updateAuthUI(){
  ...
  if (authToken && authUser){
    if (userStatus) userStatus.textContent = `👤 ${authUser.email} (${authUser.role})`;
    if (logoutBtn) logoutBtn.classList.remove("hidden");
    if (authWho) authWho.textContent = `Signed in as: ${authUser.email}`;
    // Enable config screen
    $("introConfig")?.classList.remove("hidden");
  } else {
    if (userStatus) userStatus.textContent = "👤 Guest";
    if (logoutBtn) logoutBtn.classList.add("hidden");
    if (authWho) authWho.textContent = "";
    // Require auth for starting game
    $("introConfig")?.classList.add("hidden");
  }
}
...
async function doLogin(email, password){
  ...
  setAuthMessage("Login successful. You can start the game.", true);
}

async function doSignup(email, password){
  const data = await apiJson("/api/auth/signup", { method:"POST", body:{ email, password } });
  setAuth(data.token, data.user);
  setAuthMessage("Account created. You can start the game.", true);
}

async function saveScoreToDB(payload){
  // payload: { guessed_lat, guessed_lng, true_lat, true_lng, score }
  const saveStatus = $("saveStatus");
  if (saveStatus) saveStatus.textContent = "Saving...";
  try{
    await apiJson("/api/scores", { method:"POST", body: payload });
    if (saveStatus) saveStatus.textContent = "Saved ✓";
  }catch(e){
    if (saveStatus) saveStatus.textContent = `Not saved (${e.message})`;
  }
}
...
function startGame() {
  // Require auth
  if (!authToken){
    showIntroAuth("Please login first to start the game.");
    return;
  }
...
function handleGuess(lat, lng) {
  ...
  if (pts > 0) {
    ...
    if (lives <= 0) {
      // still save attempt below, then end
      // (we save regardless of correct/incorrect)
      // continue...
    }
  }

  // ✅ Save to DB (required by rubric): store guess & score points
  // Keep only the keys your backend likely expects.
  saveScoreToDB({
    guessed_lat: Number(lat.toFixed(6)),
    guessed_lng: Number(lng.toFixed(6)),
    true_lat: Number(target.lat.toFixed(6)),
    true_lng: Number(target.lng.toFixed(6)),
    score: pts
  });
...
function updateAnalyticsPanel() {
  ...
  statsChart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: "Distance (km)", data: distances, yAxisID: "y" },
        { label: "Points", data: points, yAxisID: "y1" },
      ],
    },
    options: {
      responsive: true,
...
function init() {
  ...
  $("difficultySelIntro").addEventListener("change", (e) => setDifficulty(e.target.value));
  ...
  $("introStart").addEventListener("click", () => {
    if (!authToken){
      setAuthMessage("Please login first.", false);
      return;
    }
  ...
  function setTab(which){
    ...
    if (which === "login"){
      ...
      panelSignup.classList.remove("visible");
    } else {
      tabSignup.classList.add("active");
      tabLogin.classList.remove("active");
      panelSignup.classList.add("visible");
  ...
  $("btnLogin").addEventListener("click", async () => {
    ...
    if (!email || !password){
      setAuthMessage("Email and password are required.", false);
      return;
    }
    try{
  ...
  $("btnSignup").addEventListener("click", async () => {
    ...
    if (password.length < 6){
      setAuthMessage("Password must be at least 6 characters.", false);
      return;
    }
    try{
      ...
      setAuthMessage(`Signup failed: ${e.message}`, false);
    }
  });
  ...
  // If already logged in, allow config; else block start.
  if (authToken && authUser){
    setAuthMessage("Welcome back. Choose settings and start.", true);
    $("introConfig").classList.remove("hidden");
  } else {
    setAuthMessage("Please login to continue.", false);
    $("introConfig").classList.add("hidden");
